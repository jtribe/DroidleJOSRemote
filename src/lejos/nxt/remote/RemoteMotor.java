...
public class RemoteMotor implements TachoMotor, NXTProtocol {
	...
	public RemoteMotor(NXTCommand nxtCommand, int id) {
		this.id = id;
		this.power = 120; // 80% power by default. Is this speed too?
	...
	public void forward() {
		...
		try {
			speed = power;
			nxtCommand.setOutputState(id, speed, this.mode + MOTORON,
	 ...
	 * like the trip odometer on your car. This count is reset each time a new
	 * function is called in Pilot.
	 * 
	 * @deprecated
	...
	/**
	 * Block Encoder Count is the count used to synchronize motors with one
	 * another. NOTE: If you are using leJOS NXJ firmware this will always
	 * return 0 because this variable is not used in in leJOS NXJ firmware. Use
	 * getRotationCount() instead.
	 * 
	 * @deprecated
	...
	public void rotate(int count, boolean returnNow) {
		this.runState = MOTOR_RUN_STATE_RUNNING;
		// ** Really this can accept a ULONG value for count. Too lazy to
		// properly convert right now:
		byte status = 0;
		// !! This used to say power > 0, apparently not working.
		// if(power > 0)
		try {
			if (count > 0)
				nxtCommand.setOutputState(id, power, this.mode + MOTORON,
						regulationMode, turnRatio, runState, count); // Note
			// using
			// tachoLimit
			// with
			// Lego
			// FW
				...
				nxtCommand.setOutputState(id, (byte) -power, this.mode
						+ MOTORON, regulationMode, turnRatio, runState, Math
						.abs(count)); // Note using tachoLimit with Lego FW
		} catch (IOException ioe) {
			System.out.println(ioe.getMessage());
		}
		if (returnNow) {
	...
	/**
	 * CURRENTLY NOT IMPLEMENTED! Use isMoving() for now. returns true when
	 * motor is rotating toward a specified angle
	 */
	 ...
	 * REGULATION_MODE_IDLE turns off regulation REGULATION_MODE_MOTOR_SPEED
	 * regulates the speed (I think) REGULATION_MODE_MOTOR_SYNC synchronizes
	 * this and any other motor with SYNC enabled.
	 * 
	 * @param mode
	...
	public void setRegulationMode(int mode) {
		// !! Consider removing this method! No need, confusing, makes other
		// forward methods unreliable.
	...
	/**
	 * Resets the block tachometer. NOTE: If you are using leJOS NXJ firmware
	 * this will not do anything because BlockTacho is not used in the leJOS NXJ
	 * firmware. Use resetRotationCounter() instead.
	 * 
	 * @deprecated
	 * @return Error value. 0 means success. See lejos.pc.comm.ErrorMessages for
	 *         details.
	 */
	public int resetBlockTacho() {
		// Note: This method can also reset tachometer relative to last
		// position.
		// I didn't include this because it seems unintuitive, but the
		// functionality could be added, maybe with a resetTachoRelative()
		// method.
		// Just change false to true in statement below for relative reset.
		// @param relative TRUE: position relative to last movement, FALSE:
		// absolute position
	...
	public void stop() {
		...
		// this.regulationMode = REGULATION_MODE_MOTOR_SPEED;
		try {
			// NOTE: Setting power to 0 seems to make it lock motor, not float
			// it.
			speed = (byte) 0;
			nxtCommand.setOutputState(id, speed, BRAKE + MOTORON + REGULATED,
					regulationMode, turnRatio, runState, 0);
		} catch (IOException ioe) {
			System.out.println(ioe.getMessage());
			// return -1;
	...
	public void flt() {
		...
		try {
			nxtCommand.setOutputState(id, (byte) 0, 0x00, regulationMode,
	...
	public int getRotationSpeed() {
		// TODO Currently dummy for remote motors - returns the speed that has
		// been set.
		return getSpeed();
	}
