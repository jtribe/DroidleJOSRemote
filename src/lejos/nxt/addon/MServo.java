...
/**
 * MServo, is a abstraction to model any RC Servo (continuous and non
 * continuous) plugged to
 * 
...
public class MServo extends I2CSensor{
	...
	/**
	 * 
	 * The initial Constructor. This constructor establish where is plugged
	 * NXTServo on NXT Brick, where the RC Servo is plugged into NXTServo
	 * 
	 * @param port
	 * @param location
	 * 
	 ...
	 * http://h10025.www1.hp.com/ewfrf/wc/fastFaqLiteDocument
	 * ?lc=es&cc=mx&docname=bsia5214&dlc=es&product=20037
	 */
	private float getLinearInterpolation(float x, float x1, float x2, float y1,
			float y2) {
		float y;
	 ...
	 * 
	 * Note:Pulse range is: 500-2500, but internally it is necessary to divide
	 * into 2
	 * 
	 * @param pulse
	 *            the pulse width
	...
	public void setAngle(int angle) {
		this.angle = angle;
		this.pulse = Math.round(getLinearInterpolation(angle, minAngle,
				maxAngle, minPulse, maxPulse));
		this.setPulse(pulse);
	}

	public void setMinMax(int min, int max, int initvalue, int pulse) {
		this.minAngle = min;
		this.maxAngle = max;
		this.pulse = pulse;
		setAngle(initvalue);
	...
	public void backward() {
		int angle = getAngle() - 1;
		angle = checkMinMaxAngle(angle);
		setAngle(angle);
	}
	...
	private int checkMinMaxAngle(int value) {
		if (value < minAngle) {
			value = minAngle;
	...
	public int addAngle(int value) {
		if (this.angle == 0) {
			...
			int newAngle = checkMinMaxAngle(angle + value);
			setAngle(newAngle);
			// if (value > 0) {
			// for (int i = 0; i < value; i++) {
			// int newAngle = angle + 1;
			// int testAngle = checkMinMaxAngle(newAngle);
			// if (newAngle != testAngle) {
			// break;
			// }
			// setAngle(newAngle);
			// }
